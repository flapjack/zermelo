  ...
  module Locks
    ...
    class RedisLock
      ...
      def extend_life( new_life )
        do_extend( new_life ) or raise Sandstorm::LockNotAcquired.new(@keys.join(", "))
      end
      ...
      def do_lock( tries = 3 )
        ...
        loop do
          ...
          result = Sandstorm.redis.msetnx(*lock_keyvals)
      ...
      def do_extend( new_life )
        ...
        with_watch( *owner_keys  ) do
          owners = Sandstorm.redis.mget( *owner_keys )
          if owners == ([@owner_value.to_s] * owner_keys.size)
            result = Sandstorm.redis.multi do |multi|
      ...
      def stale_key?
        ...
        with_watch( *full_keys ) do
          ...
          owners_expires = Sandstorm.redis.mget(full_keys)
      ...
      def with_watch( *args, &block )
        Sandstorm.redis.watch( *args )
        begin
          block.call
        ensure
          Sandstorm.redis.unwatch
        end
      end
