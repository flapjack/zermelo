...
module Zermelo
  ...
  class << self
    ...
    %i[redis influxdb].each do |backend|
      define_method(backend) do
        Thread.current["zermelo_#{backend}".to_sym]
      end
    end

    def redis=(connection)
      Thread.current[:zermelo_redis] =
        if connection.nil?
          nil
        else
  ...
  class ConnectionProxy
    ...
    #   @proxied_connection.respond_to?(method_name, include_private)
    # end

    # Ruby 1.9 or greater
    def respond_to_missing?(method_name, include_private = false)
      @proxied_connection.respond_to?(method_name, include_private) || super
    end

    def method_missing(method_name, *args, &block)
      return super unless @proxied_connection.respond_to?(method_name)
      Zermelo.logger.debug("#{method_name}#{args.empty? ? '' : ' ' + args.inspect}") unless Zermelo.logger.nil?
      result = @proxied_connection.send(method_name, *args, &block)
      Zermelo.logger.debug("#{method_name} result: #{result}") unless Zermelo.logger.nil?
      result
    end
  end
end
