  ...
  module Associations
    class Multiple
      ...
      def remove(*records)
        ...
        raise 'Invalid record class' if records.any? { |r| !r.is_a?(@associated_class) }
        raise 'Record(s) must have been saved' unless records.all?(&:persisted?) # may need to be moved
        @parent_klass.lock(*@lock_klasses) do
      ...
      class << self
        ...
        def associated_ids_for(backend, type, klass, name, *these_ids)
          key_type = case type
                     when :has_many, :has_and_belongs_to_many
                       :set
                     when :has_sorted_set
          ...
          if ba.nil? || !opts[:callbacks] || !@parent_klass.respond_to?(ba) ||
            ...
            when :has_many
              ...
              record_ids.each do |record_id|
                inverse_copy = _inverse.clone
                inverse_copy.id = record_id
                @backend.add(inverse_copy, "#{@inverse}_id" => @parent_id)
              end
            when :has_sorted_set
              # inverse is belongs_to
              record_ids.each do |score_and_record_id|
                inverse_copy = _inverse.clone
                inverse_copy.id = score_and_record_id.last
                @backend.add(inverse_copy, "#{@inverse}_id" => @parent_id)
              end
            when :has_and_belongs_to_many
              ...
              record_ids.each do |record_id|
                inverse_copy = _inverse.clone
                inverse_copy.id = record_id
                @backend.add(inverse_copy, @parent_id)
              end
            end
          ...
          if br.nil? || !opts[:callbacks] || !@parent_klass.respond_to?(br) ||
            ...
            when :has_many, :has_sorted_set
              ...
              record_ids.each do |record_id|
                inverse_copy = _inverse.clone
                inverse_copy.id = record_id
                @backend.delete(inverse_copy, "#{@inverse}_id")
              end
            when :has_and_belongs_to_many
              # inverse is has_and_belongs_to_many
              record_ids.each do |record_id|
                inverse_copy = _inverse.clone
                inverse_copy.id = record_id
                @backend.delete(inverse_copy, @parent_id)
              end
            end
