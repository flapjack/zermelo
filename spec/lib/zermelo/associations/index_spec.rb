...
describe Zermelo::Associations::Index do # rubocop:disable Metrics/BlockLength
  context 'redis', redis: true do # rubocop:disable Metrics/BlockLength
    ...
    it 'adds an entry to a set indexing an attribute' do
      example = ZermeloExamples::RedisIndex.new(id: '1', emotion: 'happy')
      expect(example).to be_valid
      expect(example.save).to be true

      expect(redis.exists('redis_index::indices:by_emotion:string:happy')).to be true
      expect(redis.smembers('redis_index::indices:by_emotion:string:happy')).to eq(['1'])
    end
    ...
    it 'removes an entry from a sorted set indexing an attribute' do
      ...
      example2 = ZermeloExamples::RedisIndex.new(id: '2', emotion: 'happy')
      example2.save

      expect(redis.smembers('redis_index::indices:by_emotion:string:happy')).to eq(%w[1 2])

      example.destroy

      expect(redis.smembers('redis_index::indices:by_emotion:string:happy')).to eq(['2'])
    end
    ...
    it 'changes an entry in a sorted set indexing an attribute' do
      ...
      expect(redis.smembers('redis_index::indices:by_emotion:string:sad')).to eq(['1'])
    end
  end
end
