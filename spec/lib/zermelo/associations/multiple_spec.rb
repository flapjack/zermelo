...
describe Zermelo::Associations::Multiple do # rubocop:disable Metrics/BlockLength
  context 'has_many' do # rubocop:disable Metrics/BlockLength
    shared_examples 'has_many functions work', has_many: true do # rubocop:disable Metrics/BlockLength
      let(:parent) do
        create_parent(id: '8')
      ...
      it 'raises an error when calling add on has_many without an argument' do
        expect { parent.children.add }.to raise_error('No records to add')
      end

      it 'raises an error when calling delete on has_many without an argument' do
        expect { parent.children.remove }.to raise_error('No records to remove')
      end

      context 'filters' do # rubocop:disable Metrics/BlockLength
        before do
        ...
        it 'by indexed attribute values' do
          ...
          expect(important_kids.map(&:id)).to eq(%w[3 4])
        end

        it 'by intersecting ids' do
          important_kids = parent.children.intersect(important: true, id: %w[4 5]).all
          expect(important_kids).not_to be_nil
          expect(important_kids).to be_a(Set)
        ...
        it 'applies chained intersect and union filters to a has_many association' do
          ...
          expect(result.map(&:id)).to eq(%w[3 4])
        end
        ...
        it 'returns associated ids for multiple parent ids' do
          create_parent(id: '9')
          parent9 = parent_class.find_by_id('9')

          create_child(parent9, id: '6', important: false)
    ...
    context 'redis', redis: true, has_many: true do # rubocop:disable Metrics/BlockLength
      ...
      def create_child(parent, attrs = {}) # rubocop:disable Metrics/AbcSize
        unless parent.nil?
          redis.sadd("#{pk}:#{parent.id}:assocs:children_ids", attrs[:id])
          redis.hmset("#{ck}:#{attrs[:id]}:assocs:belongs_to", { 'parent_id' => parent.id }.to_a.flatten)
        end
        redis.hmset("#{ck}:#{attrs[:id]}:attrs", { 'important' => attrs[:important] }.to_a.flatten)
        redis.sadd("#{ck}::indices:by_important:boolean:#{![FalseClass, NilClass].include?(attrs[:important].class)}",
                   attrs[:id])
      ...
      it 'queries using association objects' do
        create_parent(id: '8')
        parent8 = parent_class.find_by_id('8')
        create_child(parent8, id: '5')
        create_child(parent8, id: '6')

        create_parent(id: '9')
        parent9 = parent_class.find_by_id('9')
        create_child(parent9, id: '7')

        create_parent(id: '10')

        assocs = parent_class.intersect(id: %w[8 10]).
                 associations_for(:children).values

        children = child_class.intersect(id: assocs)
        expect(children.count).to eq(2)
        expect(children.ids).to eq(Set.new(%w[5 6]))
      end
      ...
      it 'queries using multiple association objects' do
        ...
        create_parent(id: '10')
        parent10 = parent_class.find_by_id('10')
        create_child(parent10, id: '4')

        children = child_class.intersect(id: [parent8.children, parent9.children])
        expect(children.count).to eq(3)
        expect(children.ids).to eq(Set.new(%w[5 6 7]))
      end
      ...
      it 'queries using a single filter object' do
        ...
        par = parent_class.intersect(id: %w[8 10])

        parent_ids = parent_class.intersect(id: par).ids
        expect(parent_ids).to eq(Set.new(%w[8 10]))
      end
      ...
      it 'queries using multiple filter objects' do
        ...
        par1 = parent_class.intersect(id: ['8'])
        par2 = parent_class.intersect(id: ['10'])

        parent_ids = parent_class.intersect(id: [par1, par2]).ids
        expect(parent_ids).to eq(Set.new(%w[8 10]))
      end
      ...
      it 'queries using a combination of bare value, association and filter object' do
        ...
        children = child_class.intersect(id: assocs + [
          parent9.children.intersect(id: '7'), '4'
        ])
        expect(children.count).to eq(4)
        expect(children.ids).to eq(Set.new(%w[4 5 6 7]))
      end
    end

    context 'influxdb', influxdb: true, has_many: true do # rubocop:disable Metrics/BlockLength
  ...
  context 'has_and_belongs_to_many' do # rubocop:disable Metrics/BlockLength
    shared_examples 'has_and_belongs_to_many functions work', has_and_belongs_to_many: true do # rubocop:disable Metrics/BlockLength
      before do
      ...
      it 'raises an error when calling add on has_and_belongs_to_many without an argument' do
        ...
        expect { primary.secondaries.add }.to raise_error('No records to add')
      end

      it 'raises an error when calling delete on has_and_belongs_to_many without an argument' do
        primary = primary_class.find_by_id('8')

        expect { primary.secondaries.remove }.to raise_error('No records to remove')
      end

      it 'deletes a record from the set' do
        create_primary(id: '9', active: false)
        primary = primary_class.find_by_id('8')
        primary2 = primary_class.find_by_id('9')
        secondary = secondary_class.find_by_id('2')

        secondary.primaries.add(primary, primary2)

        expect(secondary.primaries.count).to eq(2)
        expect(primary.secondaries.count).to eq(1)
        expect(primary2.secondaries.count).to eq(1)
        ...
        expect(primary2.secondaries.ids).to eq(Set.new(['2']))
      end
      ...
      it 'clears all records from the set' do
        ...
        expect(primary.secondaries.count).to eq(0)
        expect(primary2.secondaries.count).to eq(0)
        expect(secondary.primaries.ids).to eq(Set.new)
        expect(primary.secondaries.ids).to eq(Set.new)
        expect(primary2.secondaries.ids).to eq(Set.new)
      end
      ...
      context 'filters' do # rubocop:disable Metrics/BlockLength
        it 'filters has_and_belongs_to_many records by indexed attribute values' do
          ...
          primary = primary_class.find_by_id('8')
          primary2 = primary_class.find_by_id('9')
          primary3 = primary_class.find_by_id('10')
          secondary = secondary_class.find_by_id('2')

          primary.secondaries << secondary
          primary2.secondaries << secondary
          primary3.secondaries << secondary
          ...
          expect(primaries.map(&:id)).to match_array(%w[8 10])
        end
        ...
        it 'finds a record through a has_and_belongs_to_many filter' do
          ...
          expect(james.id).to eq(primary2.id)
        end
        ...
        it 'returns associated ids for multiple parent ids' do
          create_primary(id: '9', active: false)
          primary9 = primary_class.find_by_id('9')

          create_primary(id: '10', active: true)
          primary10 = primary_class.find_by_id('10')

          create_secondary(id: '3')
          create_secondary(id: '4')

          secondary2 = secondary_class.find_by_id('2')
          secondary3 = secondary_class.find_by_id('3')
          secondary4 = secondary_class.find_by_id('4')

          primary9.secondaries.add(secondary2)
          primary10.secondaries.add(secondary3, secondary4)

          assoc_ids = primary_class.intersect(id: %w[8 9 10]).
    ...
    context 'redis', redis: true, has_and_belongs_to_many: true do # rubocop:disable Metrics/BlockLength
      let(:redis) { Zermelo.redis }
      ...
      def create_primary(attrs = {}) # rubocop:disable Metrics/AbcSize
        redis.sadd("#{pk}::attrs:ids", attrs[:id])
        redis.hmset("#{pk}:#{attrs[:id]}:attrs", { 'active' => attrs[:active] }.to_a.flatten)
        redis.sadd("#{pk}::indices:by_active:boolean:#{![FalseClass, NilClass].include?(attrs[:active].class)}",
                   attrs[:id])
      end
  ...
  context 'has_sorted_set' do # rubocop:disable Metrics/BlockLength
    ...
    context 'redis', redis: true, has_sorted_set: true do # rubocop:disable Metrics/BlockLength
      let(:redis) { Zermelo.redis }
      ...
      def create_child(parent, attrs = {}) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
        unless parent.nil?
          redis.zadd("#{pk}:#{parent.id}:assocs:children_ids",
                     attrs[:timestamp].to_f, attrs[:id])
        end

        redis.hmset("#{ck}:#{attrs[:id]}:attrs", {
          'emotion' => attrs[:emotion],
          'timestamp' => attrs[:timestamp].to_f
        }.to_a.flatten)

        redis.sadd("#{ck}::indices:by_emotion:string:#{attrs[:emotion]}", attrs[:id])
        unless parent.nil?
          redis.hmset("#{ck}:#{attrs[:id]}:assocs:belongs_to",
                      { 'parent_id' => parent.id }.to_a.flatten)
        end
        redis.zadd("#{ck}::attrs:ids", attrs[:timestamp].to_f, attrs[:id])
      end

      def create_reversed_child(parent, attrs = {}) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
        unless parent.nil?
          redis.zadd("#{pk}:#{parent.id}:assocs:reversed_children_ids",
                     attrs[:timestamp].to_f, attrs[:id])
        end
        ...
        unless parent.nil?
          redis.hmset("#{ck}:#{attrs[:id]}:assocs:belongs_to",
                      { 'reversed_parent_id' => parent.id }.to_a.flatten)
        end
        redis.zadd("#{ck}::attrs:ids", attrs[:timestamp].to_f, attrs[:id])
      end
      ...
      it 'sets a parent/child has_sorted_set relationship between two records in redis' do
        child = child_class.new(id: '4', emotion: 'indifferent', timestamp: time)
        expect(child.save).to be_truthy
        ...
        expect(redis.hgetall("#{ck}:4:attrs")).
          to eq('emotion' => 'indifferent', 'timestamp' => time.to_f.to_s)
        expect(redis.hgetall("#{ck}:4:assocs:belongs_to")).to eq('parent_id' => '8')

        result = redis.zrange("#{pk}:8:assocs:children_ids", 0, -1, with_scores: true)
        # .should == [['4', time.to_f]]
      ...
      it 'clears the belongs_to association when the parent record is deleted' do
        create_child(parent, id: '6', timestamp: time, emotion: 'upset')
      ...
      it 'sets the score in a sorted set appropriately when assigned from the belongs_to' do
        create_child(nil, id: '4', timestamp: time - 20, emotion: 'upset')

        child = child_class.find_by_id!('4')
        child.parent = parent

        expect(redis.zrange("#{pk}:8:assocs:children_ids", 0, -1, with_scores: true)).
          to eq([['4', (time - 20).to_f]])
      end

      it 'returns the first record' do
        create_child(parent, id: '4', timestamp: time - 20, emotion: 'upset')
        create_child(parent, id: '5', timestamp: time - 10, emotion: 'happy')
        create_child(parent, id: '6', timestamp: time, emotion: 'upset')
      ...
      it 'returns the last record for first if reversed' do
        create_reversed_child(parent, id: '4', timestamp: time - 20, emotion: 'upset')
        create_reversed_child(parent, id: '5', timestamp: time - 10, emotion: 'happy')
        create_reversed_child(parent, id: '6', timestamp: time, emotion: 'upset')
      ...
      it 'returns associated ids for multiple parent ids' do
        ...
        create_child(parent, id: '3', timestamp: time - 20, emotion: 'ok')
        create_child(parent, id: '4', timestamp: time - 10, emotion: 'ok')
        create_child(parent10, id: '5', timestamp: time, emotion: 'not_ok')

        assoc_ids = parent_class.intersect(id: %w[8 9 10]).
                    associated_ids_for(:children)
        expect(assoc_ids).to eq('8'  => Zermelo::OrderedSet.new(%w[3 4]),
                                '9'  => Zermelo::OrderedSet.new,
                                '10' => Zermelo::OrderedSet.new(%w[5]))
      end
      ...
      context 'filters' do # rubocop:disable Metrics/BlockLength
        ...
        it 'by indexed attribute values' do
          ...
          expect(upset_children.map(&:id)).to eq(%w[4 6])
        end
        ...
        it 'a subset of a sorted set by index' do
          ...
          expect(children.map(&:id)).to eq(%w[4 5])
        end
        ...
        it 'a reversed subset of a sorted set by index' do
          ...
          expect(children).to be_a(Zermelo::OrderedSet)
          expect(children.size).to eq(2)
          expect(children.map(&:id)).to eq(%w[6 5])
        end
        ...
        it 'a reversed subset of a sorted set by score' do
          ...
          expect(children).to be_a(Zermelo::OrderedSet)
          expect(children.size).to eq(2)
          expect(children.map(&:id)).to eq(%w[5 4])
        end
        ...
        it 'ANDs multiple union arguments, not ORs them' do
          children = parent.children.intersect(id: %w[4]).
                     union(emotion: 'upset', id: %w[4 6]).all
          expect(children).not_to be_nil
          expect(children).to be_a(Zermelo::OrderedSet)
          expect(children.size).to eq(2)
          expect(children.map(&:id)).to eq(%w[4 6])
        end

        it 'ANDs multiple diff arguments, not ORs them' do
          children = parent.children.diff(emotion: 'upset', id: %w[4 5]).all
          expect(children).not_to be_nil
          expect(children).to be_a(Zermelo::OrderedSet)
          expect(children.size).to eq(2)
          expect(children.map(&:id)).to eq(%w[5 6])
        end
        ...
        it 'the exclusion of a sorted set by index' do
          ...
          expect(children).to be_a(Zermelo::OrderedSet)
          expect(children.size).to eq(1)
          expect(children.map(&:id)).to eq(%w[6])
        end
