...
describe Zermelo::Associations::UniqueIndex do # rubocop:disable Metrics/BlockLength
  context 'redis', redis: true do # rubocop:disable Metrics/BlockLength
    ...
    it 'adds an entry to a hash indexing an attribute' do
      example = ZermeloExamples::RedisUniqueIndex.new(id: '1', name: 'John Smith')
      expect(example).to be_valid
      expect(example.save).to be true

      expect(redis.exists('redis_unique_index::indices:by_name')).to be true
      expect(redis.hgetall('redis_unique_index::indices:by_name')).
        to eq('string:John%20Smith' => '1')
    end
    ...
    it 'removes an entry from a hash indexing an attribute' do
      ...
      example2 = ZermeloExamples::RedisUniqueIndex.new(id: '2', name: 'Roger Wilco')
      example2.save

      expect(redis.hgetall('redis_unique_index::indices:by_name')).
        to eq('string:John%20Smith' => '1', 'string:Roger%20Wilco' => '2')

      example.destroy

      expect(redis.hgetall('redis_unique_index::indices:by_name')).
        to eq('string:Roger%20Wilco' => '2')
    end
